...
///// Removed L and U from LG_FREE_ALL here
//#undef  LG_FREE_ALL
//#define LG_FREE_ALL                         \
//{                                           \
//    GrB_free (&C) ;                         \
...
/// Changed by Zhen Peng on 01/03/2023
/// Moved L and U initialization out of timing
...
//  GrB_Matrix C = NULL, L = NULL, U = NULL, T = NULL ;
  ...
  LG_ASSERT_MSG (
      ...
      method == LAGr_TriangleCount_Burkhardt_NoMask  ||  // 7: sum (sum ((A^2) .* A)) / 6, no masking, elementwise multiplication
      method == LAGr_TriangleCount_Cohen_NoMask      ||  // 8: sum (sum ((L * U) .*A))/2, no masking, elementwise multiplication
      method == LAGr_TriangleCount_Sandia_LL_NoMask  ||  // 9: sum (sum ((L * L) .* L)), no masking, elementwise multiplication
      method == LAGr_TriangleCount_Sandia_UU_NoMask  ||  // 10: sum (sum ((U * U) .* U)), no masking, elementwise multiplication
      method == LAGr_TriangleCount_Sandia_LUT_NoMask ||  // 11: sum (sum ((L * U') .* L)), no masking, elementwise multiplication
      method == LAGr_TriangleCount_Sandia_ULT_NoMask,    // 12: sum (sum ((U * L') .* U)), no masking, elementwise multiplication
      GrB_INVALID_VALUE, "method is invalid") ;

  /// Turn off presort by Zhen Peng on 01/31/2023
//    // get the presort
...
#endif
  /// Changed to GrB_PLUS_MONOID_FP64 by Zhen Peng on 01/31/2023
  GrB_Monoid monoid = GrB_PLUS_MONOID_FP64 ;
...
//      LG_TRY (tricount_prep (&L, &U, A, msg)) ;
      ...
      /// Added by Zhen Peng on 12/5/2022
    case LAGr_TriangleCount_Burkhardt_NoMask:  // 7: sum (sum ((A^2) .* A)) / 6, no masking, elementwise multiplication

      GRB_TRY (GrB_mxm (C, NULL, NULL, semiring, A, A, GrB_DESC_S)) ;
      GRB_TRY (GrB_eWiseMult(C, NULL, NULL, GrB_TIMES_FP64, C, A, NULL)) ;
//            GRB_TRY (GrB_eWiseMult(C, NULL, NULL, GxB_LAND_FP64, C, A, NULL)) ;
//            GRB_TRY (GrB_eWiseMult(C, NULL, NULL, GxB_PAIR_FP64, C, A, NULL)) ;
//            GRB_TRY (GrB_eWiseMult(C, NULL, NULL, GrB_TIMES_INT64, C, A, NULL)) ;
      ...
      ntri /= 6 ;
      break ;
    case LAGr_TriangleCount_Cohen_NoMask: // 8: sum (sum ((L * U) .* A)) / 2, no masking, elementwise multiplication

//      LG_TRY (tricount_prep (&L, &U, A, msg)) ;
      GRB_TRY (GrB_mxm (C, NULL, NULL, semiring, L, U, GrB_DESC_S)) ;
      GRB_TRY (GrB_eWiseMult(C, NULL, NULL, GrB_TIMES_FP64, C, A, NULL)) ;
      GRB_TRY (GrB_reduce (&ntri, NULL, monoid, C, NULL)) ;
      ntri /= 2 ;
      break ;

    case LAGr_TriangleCount_Sandia_LL_NoMask: // 9: sum (sum ((L * L) .* L)), no masking, elementwise multiplication

      // using the masked saxpy3 method
//      LG_TRY (tricount_prep (&L, NULL, A, msg)) ;
      GRB_TRY (GrB_mxm (C, NULL, NULL, semiring, L, L, GrB_DESC_S)) ;
      GRB_TRY (GrB_eWiseMult(C, NULL, NULL, GrB_TIMES_FP64, C, L, NULL)) ;
      GRB_TRY (GrB_reduce (&ntri, NULL, monoid, C, NULL)) ;
      break ;

    case LAGr_TriangleCount_Sandia_UU_NoMask: // 10: sum (sum ((U * U) .* U)), no masking, elementwise multiplication

      // using the masked saxpy3 method
//      LG_TRY (tricount_prep (NULL, &U, A, msg)) ;
      GRB_TRY (GrB_mxm (C, NULL, NULL, semiring, U, U, GrB_DESC_S)) ;
      GRB_TRY (GrB_eWiseMult(C, NULL, NULL, GrB_TIMES_FP64, C, U, NULL)) ;
      GRB_TRY (GrB_reduce (&ntri, NULL, monoid, C, NULL)) ;
      break ;

    case LAGr_TriangleCount_Sandia_LUT_NoMask: // 11: sum (sum ((L * U') .* L)), no masking, elementwise multiplication
      ...
      // using the masked dot product
//      LG_TRY (tricount_prep (&L, &U, A, msg)) ;
      GRB_TRY (GrB_mxm (C, NULL, NULL, semiring, L, U, GrB_DESC_ST1)) ;
      GRB_TRY (GrB_eWiseMult(C, NULL, NULL, GrB_TIMES_FP64, C, L, NULL)) ;
      GRB_TRY (GrB_reduce (&ntri, NULL, monoid, C, NULL)) ;
      break ;

    case LAGr_TriangleCount_Sandia_ULT_NoMask: // 12: sum (sum ((U * L') .* U)), no masking, elementwise multiplication

      // using the masked dot product
//      LG_TRY (tricount_prep (&L, &U, A, msg)) ;
      GRB_TRY (GrB_mxm (C, NULL, NULL, semiring, U, L, GrB_DESC_ST1)) ;
      GRB_TRY (GrB_eWiseMult(C, NULL, NULL, GrB_TIMES_FP64, C, U, NULL)) ;
      GRB_TRY (GrB_reduce (&ntri, NULL, monoid, C, NULL)) ;
      break ;
  }
